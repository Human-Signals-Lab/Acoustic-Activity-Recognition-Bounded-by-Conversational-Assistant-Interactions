...
def train(args):
    ...
    if model == 'Cnn6' or model =='FineTuneCNN14' or  model == 'FineTuneCnn6' or model == 'FineTuneCnn6_FC2Classifier' or model == 'FineTuneCNN14_UpdateAll' or model == 'FineTuneCNN14_BiGRU':
        ...
        loo = LeaveOneOut()
        results_path = './results/sample_rate={},window_size={},hop_size={},mel_bins={},fmin={},fmax={}/data_type={}/LOPO+1/segment_{}_{}_interaction_based_{}_test_segment_{}/pad_{}/{}_batch_size={}/{}'.format(sr, window_size, hop_size, mel_bins, fmin, fmax, data_type,segment, segment_size, interaction_based, test_segment, pad, model_name, batch_size,context_location)
        ...
        for p_train, p_test in loo.split(data.participants):


            train = np.asarray(data.participants)[p_train.astype(int)]
            test = np.asarray(data.participants)[p_test.astype(int)][0]
            print(test)
            for train_index, test_index in loo.split(test.session_data()):
                ...
                checkpoints_dir = './checkpoints/sample_rate={},window_size={},hop_size={},mel_bins={},fmin={},fmax={}/data_type={}/LOPO+1/segment_{}_{}_interaction_based_{}_test_segment_{}/pad_{}/{}_{}/participant_{}/batch_size={}'.format(
                ...
                statistics_path = './statistics/sample_rate={},window_size={},hop_size={},mel_bins={},fmin={},fmax={}/data_type={}/LOP0+1/segment_{}_{}_interaction_based_{}_test_segment_{}/pad_{}/{}_{}/participant_{}/batch_size={}/statistics.pkl'.format(
                            sr, window_size, hop_size, mel_bins, fmin, fmax, data_type, segment, segment_size, interaction_based, test_segment, pad, model_name,context_location,test.name, batch_size)
                ...
                x_tr, y_tr = test.session_data()[int(train_index)].data()
                X_train.extend(x_tr)
                y_train.extend(y_tr)
                X_test = []
                y_test = []
                audio, label = test.session_data()[int(test_index)].data()
                X_test.extend(audio)
                y_test.extend(label)
                ...
                PATH = './models_personalized_LOPO+1/sample_rate={},window_size={},hop_size={},mel_bins={},fmin={},fmax={}/data_type={}/segment_{}_{}_interaction_based_{}_test_segment_{}/pad_{}/{}_{}/batch_size={},lr={}'.format(
                ...
                results.writerow([str(test.name), str(precision), str(recall), str(fscore), str(auc)])
