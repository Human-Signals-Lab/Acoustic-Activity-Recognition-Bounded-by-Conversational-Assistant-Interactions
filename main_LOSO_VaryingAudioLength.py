...
def train(args):
    ...
    if model == 'Cnn6' or model =='FineTuneCNN14' or  model == 'FineTuneCnn6' or model == 'FineTuneCnn6_FC2Classifier' or model == 'FineTuneCNN14_UpdateAll' or model == 'FineTuneCNN14_BiGRU':
        ...
        for clip_size in [3, 5, 10, 15, 20]:

            data, sr = load_data(clip_size,args)


            loo = LeaveOneOut()

            results_path = './results/sample_rate={},window_size={},hop_size={},mel_bins={},fmin={},fmax={}/data_type={}/VaryingClipSize/LOSO/segment_{}_{}_interaction_based_{}_test_segment_{}/pad_{}/{}_batch_size={}/{}'.format(sr, window_size, hop_size, mel_bins, fmin, fmax, data_type,segment, segment_size, interaction_based, test_segment, pad, model_name, batch_size,context_location)
            if not os.path.exists(results_path):
                os.makedirs(results_path)

            file = open(results_path + 'performance_results_clipSize_' +str(clip_size) + '.csv', "w")
            ...
            for p in data.participants:
                ...
                for train_index, test_index in loo.split(p.session_data()):
                    ...
                    dd = p.session_data()
                    test = dd[int(test_index)]
                    train = dd[int(train_index)]
                    print(np.shape(test), np.shape(train))


                    checkpoints_dir = './checkpoints/sample_rate={},window_size={},hop_size={},mel_bins={},fmin={},fmax={}/data_type={}/LOSO/segment_{}_{}_interaction_based_{}_test_segment_{}/pad_{}/{}_{}/participant_{}/batch_size={}'.format(
                                sr, window_size, hop_size, mel_bins, fmin, fmax, data_type,segment, segment_size, interaction_based, test_segment, pad, model_name,context_location,p.name,batch_size)
                    if not os.path.exists(checkpoints_dir):
                        os.makedirs(checkpoints_dir)
        
                    statistics_path = './statistics/sample_rate={},window_size={},hop_size={},mel_bins={},fmin={},fmax={}/data_type={}/LOSO/segment_{}_{}_interaction_based_{}_test_segment_{}/pad_{}/{}_{}/participant_{}/batch_size={}/statistics.pkl'.format(
                                sr, window_size, hop_size, mel_bins, fmin, fmax, data_type, segment, segment_size, interaction_based, test_segment, pad, model_name,context_location,p.name, batch_size)
    ...
    #                print("Train: ", Counter(y_train))
                    ...
                    PATH = './models_personalized/Variable_ClipSize/clipSize_{}/sample_rate={},window_size={},hop_size={},mel_bins={},fmin={},fmax={}/data_type={}/segment_{}_{}_interaction_based_{}_test_segment_{}/pad_{}/{}_{}/batch_size={},lr={}'.format(
                    clip_size, sr, window_size, hop_size, mel_bins, fmin, fmax, data_type, segment, segment_size, interaction_based, test_segment, pad, model_name, context_location, batch_size, learning_rate)

                    if not os.path.exists(PATH):
                        os.makedirs(PATH)
                    PATH = PATH + '/{}_{}.pth'.format(model_name,p.name)
                    ...
                    results.writerow([str(p.name), str(precision), str(recall), str(fscore), str(auc)])
                    ...
                    plot_confusion_matrix(C, class_list=list(Enc.categories_[0]), normalize=False, title='Predicted Results - {}'.format(p.name))

                    plotCNNStatistics(statistics_path, p)
